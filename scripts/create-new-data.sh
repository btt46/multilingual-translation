...
grep ^H ${TRANSLATION_DATA}/result.vi | cut -f3 > ${TRANSLATION_DATA}/data.vi
grep ^H ${TRANSLATION_DATA}/result.en | cut -f3 > ${TRANSLATION_DATA}/data.en

# 普通文字に戻す
cat ${TRANSLATION_DATA}/data.vi | sed -r 's/(@@ )|(@@ ?$)//g'  > $NEW_DATA/new.vi
cat ${TRANSLATION_DATA}/data.en  | sed -r 's/(@@ )|(@@ ?$)//g' > $NEW_DATA/new.en
